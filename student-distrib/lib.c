...
{	
	if(c == '\n') { 
		...
		cursor_y++;
	        ...
	        *(uint8_t *)(video_mem + ((NUM_COLS*cursor_y + cursor_x) << 1)) = c;
	        *(uint8_t *)(video_mem + ((NUM_COLS*cursor_y + cursor_x) << 1) + 1) = ATTRIB;
...
*			int8_t* buf = allocated buffer to place z in
...
void* copy_to_user(void* dest, const void* src, uint32_t n) { 
